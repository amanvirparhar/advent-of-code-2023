...
def appendToMap(i, j, currNum):
	if f"{i},{j}" in gear_map:
		gear_map[f"{i},{j}"].append(int(currNum))
	else:
		gear_map[f'{i},{j}'] = [int(currNum)]
...
def checkAround(currNum, lines, i, j):
	...
	if j - len(currNum) >= 0 and not lines[i][j - len(currNum)].isdigit() and lines[i][j - len(currNum)] == '*':
		appendToMap(i, j - len(currNum), currNum)
		return True
	
	if j + 1 < len(lines[i]) and not lines[i][j + 1].isdigit() and lines[i][j + 1] == '*':
		appendToMap(i, j + 1, currNum)
		return True 

	if i - 1 >= 0:
		for z, char in enumerate(lines[i - 1][max(0, j - len(currNum)):min(j + 2, len(lines[i - 1]))]):
			if not char.isdigit() and char == '*':
				appendToMap(i - 1, max(0, j - len(currNum)) + z, currNum)
				return True
...
for gear in gear_map:
	if len(gear_map.get(gear)) == 2:
		p = 1
		for v in gear_map.get(gear):
			p *= v
		t += p
